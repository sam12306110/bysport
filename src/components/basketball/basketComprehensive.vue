                  ...
                  <table frame="void" cellpadding="0" cellspacing="0" v-for="(item,key) in race" id="stf">
                    ...
                    <tbody v-for="items in item">
                      ...
                      <td align="right" style="width: 135px;">
                        ...
                        <a href="javascript:;"
                           @click="ClickBet('篮球单式',key,items.Match_ID,items.Match_Date,{zhu:items.Match_Master,ke:items.Match_Guest},items.Match_Master,items.Match_Ho,'1','Match_Ho','盘口: '+(items.Match_ShowType=='H' ? '主让' :'客让')+'('+items.Match_RGG+')')"  v-if="items.Match_Ho!=null&&items.Match_Ho!='0'">
                      ...
                      <td  align="right" style="width: 133px;">
                        ...
                        <a href="javascript:" title="大"
                           @click="ClickBet('篮球单式',key,items.Match_ID,items.Match_Date,{zhu:items.Match_Master,ke:items.Match_Guest},items.Match_DxGG1,items.Match_DxDpl,'1','Match_DxDpl')"  v-if="items.Match_DxDpl!=null&&items.Match_DxDpl!='0'">
                      ...
                      <td align="right">
                        ...
                        <a href="javascript:;"
                           @click="ClickBet('篮球单式',key,items.Match_ID,items.Match_Date,{zhu:items.Match_Master,ke:items.Match_Guest},items.Match_Guest,items.Match_Ao,'1','Match_Ao','盘口: '+(items.Match_ShowType=='H' ? '主让' :'客让')+'('+items.Match_RGG+')')"  v-if="items.Match_Ao!=null&&items.Match_Ao!='0'">
                          <font>{{items.Match_Ao}}</font></a>
                      </td>
                      <td align="right">
                        <span class="qiu" v-if="items.Match_DxXpl!=null&&items.Match_DxXpl!='0'">{{items.Match_DxGG2}}</span>
                        <a href="javascript:"  title="小" @click="ClickBet('篮球单式',key,items.Match_ID,items.Match_Date,{zhu:items.Match_Master,ke:items.Match_Guest},items.Match_DxGG2,items.Match_DxXpl,'1','Match_DxXpl')" v-if="items.Match_DxXpl!=null&&items.Match_DxXpl!='0'">
